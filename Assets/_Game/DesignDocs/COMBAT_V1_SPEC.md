...
- `BossBarragePocketReviewOwner` is the review-only pocket result owner for the first lane slice: defeat the close threat, receive a short `0.9s` blocker-break relief beat based on the collected pressure-relief data, then spend `SummonSlot1` while surviving boss pressure. It stops EN gain and boss pressure after clear/fail so the review state does not keep running behind the result.
