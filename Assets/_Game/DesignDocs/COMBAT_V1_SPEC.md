...
- `BossBarragePocketCameraCueBridge` and `BossBarragePocketVfxCueBridge` are review-only bridges from pocket result events into presentation cues. They let the follow-up window, confirmed `Skill1` hit, and missed follow-up read as short camera and in-world VFX beats without making the general camera/VFX drivers depend on encounter/test ownership.
