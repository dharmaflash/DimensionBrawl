...
- Editor validation for canonical scenes may verify required references, promoted asset ownership, prefab links, team ownership, animation trigger existence, and required scene anchors.
...
- Reapply tools may repair explicit canonical bindings, but authored Inspector values remain the source of truth for action feel and camera feel.
- Canonical scenes use assigned Input Actions instead of relying on missing-action fallbacks.

Current stabilization debt:

- One-time Review scene seeding and policy-report generators are retired; canonical scenes and focused read-only reporters are the maintained source of truth.
...
- `SummonPressureScreenPresenter` is the current screen readability layer. It owns only the proxy-local shield visual, tier-aware activation color, intercept flash, short intercept punch, final-hit linger, and the `SummonProjectileIntercept` contact cue. Real projectile blocks place that cyan-white marker at the observed projectile position and face it along the incoming normal; transform-less beam blocks use a bounded screen-surface fallback. The contact cue is distinct from the encounter-level gold `SummonBlockOpportunity` visual and carries exactly one copy of the existing reviewed block SFX bank.
...
- `BossBarragePocketCameraCueBridge` and `BossBarragePocketVfxCueBridge` are presentation-only bridges from encounter events into authored cues. They let the close-threat defeat summon-block opportunity, follow-up window, confirmed `Skill1` hit, and missed follow-up read as short in-world VFX or camera beats without giving the general camera/VFX drivers encounter ownership. A positive `SummonFollowupHitConfirmed` also requests exactly one canonical boss full-body reaction (`Hit` at LV1~2, `HitHeavy` at LV3 with `Hit` fallback) through the explicitly bound `BossBarrageVisualCueDriver`; the bridge does not replay generic hit VFX/SFX or add global time-scale, FOV, or camera ownership.
...
- `OlympusCorridorInvasionStage.unity` and `OlympusStationCombatStage.unity` are the canonical runtime scenes for the fixed rear camera, player boundary, EN gain, boss barrage, Skill1, summons, and clear/fail flow.
- `BossBarrageEncounterController` is the canonical progression and route-result owner for the fixed-rear boss-barrage encounter: defeat the close threat, receive a short blocker-break relief beat from `SummonOpportunityWindowProfile` (`DB_SummonOpportunity_BossPressureBlock`), emit a summon-block opportunity event for in-world readability, then spend `SummonSlot1` and block boss pressure. A correct post-threat summon block opens the profile-authored follow-up window, grants the profile-authored EN follow-up pulse, observes whether the follow-up `Skill1` actually damages the boss proxy, and starts the profile-authored boss-pressure break relief beat. The encounter requires that confirmed follow-up boss hit before clear; if the player misses the follow-up, boss pressure resumes and a later `SummonSlot1` pressure block can reopen the Skill1 window. This keeps the exchange readable as local defense into summon block into confirmed follow-up pressure instead of an instant checklist or button-only HUD state. It stops EN gain, boss barrage, boss cost gain, and boss costed actions after clear/fail so combat state does not keep running behind the result.
- `CombatHudPresenter` is the sole in-combat HUD and reticle surface. `CombatSessionOverlayPresenter` owns pause, settings, and combat-result routing.
