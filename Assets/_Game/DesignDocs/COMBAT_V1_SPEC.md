...
- `SummonPressureScreen` is the first narrow summon-side answer to boss projectile pressure. `SummonSlot1` opens a short-lived ally-owned screen from its frontline proxy, and higher EN tiers increase the same screen concept's intercept budget/radius/lifetime without introducing a full summon roster or boss phase manager. The screen uses trigger contact plus a small bounded overlap scan so already-overlapping hostile boss projectiles are absorbed reliably, then `PlayerSummonSlot1Action` answers with a short summon-owned counter bolt so the exchange reads as block-then-return-fire.
- `SummonPressureScreenPresenter` is the current screen readability layer. It owns only the proxy-local shield visual, activation pulse, intercept flash, short intercept punch, and final-hit linger so players can see the summon answering boss pressure without leaning on HUD text.
- `ActionCameraCueDriver` listens to `SummonSlot1` spend and pressure-screen block events as presentation-only reads. A successful summon block should request a short additive camera cue instead of depending on HUD text or moving camera ownership into summon gameplay code.
...
- `BossBarragePocketReviewOwner` is the review-only pocket result owner for the first lane slice: defeat the close threat, receive a short `0.9s` blocker-break relief beat based on the collected pressure-relief data, then spend `SummonSlot1` and block boss pressure. A correct post-threat summon block opens a `1.4s` follow-up window and a `2.4s` boss-pressure break relief beat before the pocket clears, so the exchange reads as a summon answer instead of an instant checklist. It stops EN gain and boss pressure after clear/fail so the review state does not keep running behind the result.
- `BossBarrageLaneReviewHud` is the temporary readable review display for HP, close-threat HP, boss HP, EN fill/readiness, forward-risk gain, current boss-barrage pattern/windup/projectile count, Skill1/SummonSlot1 readiness, active summon proxy/projectile/shield block state, and pocket result state.
...
- One boss-barrage pocket can be cleared, paused, or meaningfully answered through player survival/positioning plus summon action, including a short visible follow-up window after a correct summon pressure block.
