...
- `BossBarragePocketCameraCueBridge` is the review-only bridge from pocket result events into presentation cues. It lets the follow-up window, confirmed `Skill1` hit, and missed follow-up read as short camera beats without making the general camera driver depend on encounter/test ownership.
