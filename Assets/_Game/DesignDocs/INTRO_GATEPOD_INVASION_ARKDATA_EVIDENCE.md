...
| Scripted jet flyby | `Assets/_Imported/SpecialSkillsEffectsPack/AllEffects/EffectsSet_2(ScriptBased)/Effects/Effect_46_CyberAirTroopInvader/Effect_46_Parts/Effect_46_JetBomber.prefab`, `Effect_46_JetStriker.prefab` | 2026-06-28 recheck: both prefabs contain `VariousTranslateMove` (`m_fowardMove: 1`, `m_power: 1`) plus `MultipleObjectsMake` shot points. | direct, but Timeline-scrub needs parent animation | Use these instead of only static `Jet_04.fbx` where a live flyby is needed. Parent transform should still be keyed in Timeline so the user can scrub and adjust the pass. |
| Scripted stealth bomber | `Assets/_Imported/SpecialSkillsEffectsPack/AllEffects/EffectsSet_2(ScriptBased)/Effects/Effect_46_CyberAirTroopInvader/Effect_46_CyberStealthBomber.prefab` | 2026-06-28 recheck: prefab contains `MultipleObjectsMake` for one bomber, `VariousTranslateMove` (`m_fowardMove: 1`, `m_power: 0.05`), and two shot-point makers delayed at `1.0s` with `100` repeated shots at `0.05s`. | direct, but Timeline-scrub needs parent animation | Best candidate for a visible bomber run if the built-in shot spray reads well in Play mode. Key the parent run for designer control; use built-in forward/shot logic as the live effect layer. |
| Scripted aerial bombing | `Assets/_Imported/SpecialSkillsEffectsPack/AllEffects/EffectsSet_2(ScriptBased)/Effects/Effect_40_AerialBombing/Effect_40_AerialBombing.prefab` | 2026-06-28 recheck: root has three `MultipleObjectsMake` groups: rising bombs `15` at `0.1s`, dropping bombs after `2.0s` with `15` at `0.15s`, and rising bomb impacts `15` at `0.1s`. | direct | Use this as the bombing-effect body instead of hand-placing individual bomb models as the main effect. |
| Older bomber air raid pattern | `Assets/_Imported/SpecialSkillsEffectsPack/AllEffects/EffectsSet_2(ScriptBased)/Effects/Effect_23_BrandishingGunFire/Effect_23_BomberAirRaid.prefab` | 2026-06-28 recheck: five bomb make points each spawn `4` objects at `0.05s`; ship mark moves forward with `VariousTranslateMove` (`m_fowardMove: 1`, `m_power: -0.5`). | fallback | Useful as a fallback composition clue, but prefer `Effect_46` and `Effect_40` because they are clearer air-raid/bomber assets. |
