...
|---|---|---|
| canonical stage spine | `PLAYABLE_STAGE_REFERENCE_SPINE_SPEC.md` proposes an optional `StageRuleSetRef`, but no production `PlayableStageDefinition` or rule asset exists | extend the one canonical spine after its predecessor gates; do not introduce a parallel stage identity |
...
1. P1-A reaches `TerminalClosed`, wins the shared latch, seals `TerminalFinalizationAuthority`, enters `TerminalFinalizing`, and rejects every new P2-A acquisition/configuration admission; after required fact, course traversal/quiescence, P1-C RunFinalization, and current-generation presentation aggregate coverage succeeds it seals `OutcomeFactsSealed`.
...
|---|---|---|
| logical route | proposed `OLYMPUS-INVASION-01`; not approved/implemented | P1-0/P1-B route shell and current evidence |
| Station segment/definition | proposed `station_entry_combat`; Station definition/binding absent | P1-B |
...
| P2-A5 active restart integration | 4 | 5 | P1-A dispatch + P1-C/P2-A/P2-B barriers | pseudo-outcome, double dispatch, stale owner | snapshot policy and full P2-B E2E restart pass |
