...
- Defeating the local close threat should produce an in-world summon-block opportunity read before the player solves boss pressure, so the loop does not depend only on HUD text.
