...
- The enemy prefab candidate menu promotes the reviewed `ClosePunish` sci-fi melee soldier scene sample into `Assets/_Game/Prefabs/Enemies/ActionFoundation/PF_Enemy_SciFiSoldier_Melee_ClosePunish.prefab` and the reviewed `GeneralDeck` scene sample into `Assets/_Game/Prefabs/Enemies/ActionFoundation/PF_Enemy_SciFiSoldier_GeneralDeck.prefab`. Both clear scene target/camera references and keep telegraph/VFX references local to the prefab. `ClosePunish` maps to the `SciFiSoldier.Melee` archetype, while `GeneralDeck` maps to the `SciFiSoldier.Ranged` archetype with the promoted `SciFiSoldier_01` rifle visual.
...
- Ranged soldier shots are currently presentation-only projectile cues layered over existing pattern hit shapes. `LinePressure` and `RetreatShot` active cues use promoted mesh VFX prefabs with authored forward travel (`3.4m` and `3.2m`) plus elevated `ProjectileCore`/`ProjectileTrail` meshes so they read as shots in the gameplay camera without adding a separate projectile damage authority.
...
- GeneralDeck soldier prefab candidate: `PF_Enemy_SciFiSoldier_GeneralDeck` is authored under `_Game/Prefabs/Enemies/ActionFoundation/`, starts from the same `ClosePunish` profile row, keeps `DB_BasicSoldier_GeneralPatternDeck` assigned, keeps target candidates empty, and uses a distinct promoted `SciFiSoldier_01 Commando` rifle visual plus assault-rifle animation controller. It is assigned to the `SciFiSoldier.Ranged` archetype for line, fan, retreat-shot, and backline role review instead of duplicating the melee MaintenanceWorker presentation.
- GeneralDeck soldier review scene: `ActionFoundationEnemyGeneralDeckReview.unity` places one prefab instance at a farther initial distance, wires the same explicit player/enemy target candidates, and gives the instance the scene camera controller for pattern camera cues.
- Basic soldier promoted animation set: the melee MaintenanceWorker keeps `Idle`, `Run`, `Attack`, `AttackCombo2`, `AttackCombo3`, `CrouchForward`, `CrouchIdleCombat`, `RepairHigh`, `RepairLow`, `TypeOnConsole`, `Turn90Right`, `Hit`, `HitHeavy`, and `Death`. The GeneralDeck rifle soldier keeps a narrower `SciFiSoldier_01` set: `IdleAimAssaultRifle`, `RunForwardAssaultRifle`, `WalkForwardAssaultRifle`, `ShootPrimaryAssaultRifle`, `ShootSecondaryAssaultRifle`, `2HitComboAssaultRifle`, `CrouchAimAssaultRifle`, `CrouchForwardAssaultRifle`, `Hit`, `HitHeavy`, and `Death`. Both sets follow the collected enemy taxonomy (`Advance`, `Windup`, `Attack`, `Recover`, `HitReact`, `Stagger`, `Death`, `SummonCall`/assist-like signals) without promoting the full source pack.
...
- These enemy animation matches are foundation-grade reads, not final content lock. The GeneralDeck prefab now uses assault-rifle clips for line/fan/backline pressure, while `RepairHigh`, `RepairLow`, and `TypeOnConsole` remain MaintenanceWorker-only substitutes for close/elite/command sample readability until dedicated elite or commander slices are reviewed.
...
- The melee and GeneralDeck soldier prefab candidates can be reviewed as reusable authored prefabs without carrying ActionFoundationTest player targets or camera references. The two candidates intentionally differ in presentation: close-punish melee uses the MaintenanceWorker visual, while GeneralDeck/ranged uses the SciFiSoldier01 rifle visual. Future scenes/encounters should inject target candidates and camera bindings explicitly.
- `ActionFoundationEnemyPrefabReview.unity` is the intended manual test scene for the close-punish baseline, and `ActionFoundationEnemyGeneralDeckReview.unity` is the intended manual test scene for the expanded general deck candidate. Press Play there to check approach, telegraph, hit reaction, death, player targeting, enemy targeting, deck row selection, and enemy camera cues without the twelve-pattern inspection lineup.
...
- VFX cue infrastructure now uses `DB_CombatVfxCues_ActionFoundation` plus promoted game-owned mesh cue prefabs under `_Game/Art/VFX/CombatCues`. The first useful cue set covers player attack start/hit, player dodge, enemy windup/active, enemy hit/death, authored soldier pattern cues, ranged soldier shot travel cues, and elite signal cues without direct raw `_Imported` prefab references.
...
- Unity batchmode enemy prefab candidate validation passes with `ActionFoundation enemy prefab candidate validation passed.` and rejects missing prefab assets, missing GeneralDeck data, scene target candidates, serialized scene camera controllers, raw `_Imported` object references, and external scene references inside the prefabs.
- Unity batchmode enemy prefab review scene validation passes with `ActionFoundation enemy prefab review scene validation passed.` and `ActionFoundation enemy GeneralDeck review scene validation passed.` and rejects scenes that have more than one soldier, lose the prefab instance connection, miss player/enemy target candidates, lose GeneralDeck assignment, or omit the scene camera binding.
- Unity PlayMode smoke tests pass with `40` tests run, `40` passed, `0` failed for movement, promoted locomotion routing, stop-settle release, fast-window `StopStep` Animator routing, CombatGirl weapon socket alignment, directional dodge motion, no-input backward dodge, five-hit timed-buffer combo routing, attack damage without global slow motion, shared target sensing/team rules, promoted MaintenanceWorker enemy visual/Animator wiring, pattern-specific enemy animation trigger data, enemy Attack/Hit/Death animation requests including fatal-damage direct death routing and grounded death bounds, enemy windup telegraph presentation, `LinePressure` lane-lock side-dodge behavior, ranged soldier projectile cue forward-travel values, authored `TrainingDeck` row commitment, selection, `FanPressure` cone hit behavior, extended general/elite pattern data, linear-run enemy role profile coverage, role-to-archetype presentation candidate coverage, scene-free melee soldier prefab candidate mapping, scene-free GeneralDeck prefab candidate data and distinct SciFiSoldier01 visual, close-punish and GeneralDeck prefab review scene wiring, elite signal animation trigger data, shared elite damage-modification behavior, authored aura receiver protection, authored summon signal activation, encounter win/fail, dodge tint feedback, short action-camera cue cleanup/FOV widening, independent camera orbit, and the lower close-rear camera preset.
