...
- Ranged soldier shots are currently presentation-only projectile cues layered over existing pattern hit shapes. `LinePressure` and `RetreatShot` active cues use promoted mesh VFX prefabs with authored forward travel (`3.4m` and `3.2m`) so they read as shots without adding a separate projectile damage authority.
