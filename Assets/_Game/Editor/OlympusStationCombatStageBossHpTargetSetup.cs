...
namespace DimensionBrawl.Editor
{
    public static class OlympusStationCombatStageBossHpTargetSetup
    {
        ...
        public static void RunBatchVerifyAllySummonTint()
        {
            try
            {
                VerifyAllySummonTintContract();
            }
            catch (Exception exception)
            {
                Debug.LogException(exception);
        ...
        private static void VerifyAllySummonTintContract()
        {
            AssetDatabase.Refresh();
            Scene scene = EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
            PlayerSummonSlot1Action summonSlot1 =
                RequireSceneComponent<PlayerSummonSlot1Action>(scene, "player summon slot1 action");
            PlayerSupportSummonSlotAction[] supportSummons =
                CollectComponents<PlayerSupportSummonSlotAction>(scene);

            List<UnityEngine.Object> summonActions = new List<UnityEngine.Object> { summonSlot1 };
            for (int i = 0; i < supportSummons.Length; i++)
            {
                if (supportSummons[i] != null)
                {
                    summonActions.Add(supportSummons[i]);
                }
            }

            string firstCapturePath = null;
            int verified = 0;
            for (int i = 0; i < summonActions.Count; i++)
            {
                UnityEngine.Object action = summonActions[i];
                DamageTeam sourceTeam = GetSerializedDamageTeam(action, "sourceTeam");
                if (!CombatTeamUtility.IsPlayerSide(sourceTeam))
                {
                    throw new InvalidOperationException(
                        $"{action.name}.sourceTeam is not player-side: {sourceTeam}.");
                }

                SummonFrontlineProxy prefab = ResolveSerializedSummonActorPrefab(action);
                if (prefab == null)
                {
                    throw new InvalidOperationException($"{action.name} has no summon actor prefab.");
                }

                string capturePath = i == 0
                    ? Path.Combine("C:/tmp", "DimensionBrawl-AllySummonTintVerify.png")
                    : null;
                VerifyAllySummonPrefabTint(action.name, prefab, sourceTeam, capturePath, out string writtenCapture);
                if (!string.IsNullOrEmpty(writtenCapture))
                {
                    firstCapturePath = writtenCapture;
                }
            ...
            Debug.Log(
                "Verified OlympusStation allied summon tint contract. " +
                $"actions={verified} capture={firstCapturePath ?? "none"}.");
        }

        private static void VerifyAllySummonPrefabTint(
            string actionName,
            SummonFrontlineProxy prefab,
            DamageTeam sourceTeam,
            string capturePath,
            out string writtenCapturePath)
        {
            writtenCapturePath = null;
            SummonFrontlineProxy instance =
                PrefabUtility.InstantiatePrefab(prefab) as SummonFrontlineProxy;
            if (instance == null)
            {
                instance = UnityEngine.Object.Instantiate(prefab);
            }

            instance.name = "__AllySummonTintVerify_" + actionName;
            try
            {
                CombatHealth health = instance.Health ?? instance.GetComponent<CombatHealth>();
                if (health == null)
                {
                    throw new InvalidOperationException($"{prefab.name} has no CombatHealth.");
                }

                SummonFrontlineProxyPresenter presenter =
                    instance.GetComponent<SummonFrontlineProxyPresenter>();
                if (presenter == null)
                {
                    throw new InvalidOperationException($"{prefab.name} has no SummonFrontlineProxyPresenter.");
                }

                Renderer[] renderers = instance.GetComponentsInChildren<Renderer>(includeInactive: true);
                if (CountVisualRenderers(renderers) == 0)
                {
                    throw new InvalidOperationException($"{prefab.name} has no mesh renderers for ally tint.");
                }

                health.ConfigureTeam(sourceTeam);
                instance.Activate(
                    Vector3.zero,
                    Vector3.forward,
                    1,
                ...
                int visualMaterialCount = CountVisualMaterials(renderers);
                int allyRuntimeMaterials = CountAllyRuntimeMaterials(renderers);
                if (allyRuntimeMaterials < visualMaterialCount)
                {
                    throw new InvalidOperationException(
                        $"{prefab.name} did not receive AllyWhiteRuntime materials on every visual renderer. " +
                        $"expected={visualMaterialCount} actual={allyRuntimeMaterials}.");
                }

                if (!string.IsNullOrWhiteSpace(capturePath))
                {
                    CaptureAllySummonTintPreview(renderers, capturePath);
                    writtenCapturePath = capturePath;
                }
            }
            finally
            {
        ...
        private static SummonFrontlineProxy ResolveSerializedSummonActorPrefab(UnityEngine.Object action)
        {
            SerializedObject serializedObject = new SerializedObject(action);
            SerializedProperty prefabProperty = serializedObject.FindProperty("summonActorPrefab");
            SummonFrontlineProxy prefab = prefabProperty != null
                ? prefabProperty.objectReferenceValue as SummonFrontlineProxy
                : null;
            if (prefab != null)
            {
                return prefab;
            ...
            SerializedProperty prefabObjectProperty = serializedObject.FindProperty("summonActorPrefabObject");
            GameObject prefabObject = prefabObjectProperty != null
                ? prefabObjectProperty.objectReferenceValue as GameObject
                : null;
            return prefabObject != null ? prefabObject.GetComponent<SummonFrontlineProxy>() : null;
        }

        private static DamageTeam GetSerializedDamageTeam(UnityEngine.Object target, string propertyName)
        {
            ...
            return (DamageTeam)property.enumValueIndex;
        }

        private static int CountVisualRenderers(Renderer[] renderers)
        {
            int count = 0;
            if (renderers == null)
            {
            ...
            {
                if (IsVisualTintRenderer(renderers[i]))
                {
                    count++;
                }
            }
        ...
        private static int CountAllyRuntimeMaterials(Renderer[] renderers)
        {
            int count = 0;
            if (renderers == null)
            {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                if (!IsVisualTintRenderer(renderer))
                {
                    continue;
                }
                ...
                for (int materialIndex = 0; materialIndex < materials.Length; materialIndex++)
                {
                    ...
                    if (material != null && material.name.Contains("AllyWhiteRuntime"))
                    {
                        count++;
                    }
                }
        ...
        private static int CountVisualMaterials(Renderer[] renderers)
        {
            int count = 0;
            if (renderers == null)
            {
        ...
        private static bool IsVisualTintRenderer(Renderer renderer)
        {
        ...
        private static void CaptureAllySummonTintPreview(Renderer[] renderers, string capturePath)
        {
            Directory.CreateDirectory(Path.GetDirectoryName(capturePath));
            Bounds bounds = CalculateBounds(renderers);
            GameObject lightObject = new GameObject("__AllySummonTintVerify_Light");
            GameObject cameraObject = new GameObject("__AllySummonTintVerify_Camera");
            ...
            try
            {
                ...
                camera.backgroundColor = new Color(0.02f, 0.028f, 0.04f, 1f);
                camera.fieldOfView = 35f;
                camera.nearClipPlane = 0.03f;
                camera.farClipPlane = 80f;
                Vector3 center = bounds.center;
                float radius = Mathf.Max(0.75f, bounds.extents.magnitude);
                camera.transform.position = center + new Vector3(0f, radius * 0.2f, -radius * 3.2f);
                camera.transform.LookAt(center + Vector3.up * (radius * 0.1f));
                ...
                Debug.Log($"Captured ally summon tint preview: {capturePath}");
            }
            finally
            {
                if (image != null)
