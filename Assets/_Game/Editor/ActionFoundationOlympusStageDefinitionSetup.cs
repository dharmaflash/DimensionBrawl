...
namespace DimensionBrawl.Editor
{
    public static class ActionFoundationOlympusStageDefinitionSetup
    {
        ...
        private const string IntroGatePodAirRaidPreludeRootName = "IntroGatePodAirRaidPreludePreview";
        private const string IntroGatePodAirRaidCameraName = "IntroGatePodAirRaidPreludePreview_Camera";
        ...
        private const string AirRaidJetModelPath = "Assets/_Imported/SpecialSkillsEffectsPack/Models/Jet_04.fbx";
        private const string AirRaidBomberModelPath = "Assets/_Imported/SpecialSkillsEffectsPack/Models/Bomber_02.fbx";
        private const string AirRaidBombModelPath = "Assets/_Imported/SpecialSkillsEffectsPack/Models/Bomb_01.fbx";
        private const string AirRaidJetTexturePath = "Assets/_Imported/SpecialSkillsEffectsPack/Textures/SimpleTextures/Bomber_Texture.png";
        private const string AirRaidWholeStrikePrefabPath = "Assets/_Imported/SpecialSkillsEffectsPack/AllEffects/EffectsSet_1(NotScriptBased)/Effects/Effect_41_Airstrike/Effect_41_Airstrike.prefab";
        private const string AirRaidBombExplosionPrefabPath = "Assets/_Imported/SpecialSkillsEffectsPack/AllEffects/EffectsSet_1(NotScriptBased)/Effects/Effect_41_Airstrike/Effect_41_Base/Effect_41_BombExplosion.prefab";
        private const string AirRaidBulletExplosionPrefabPath = "Assets/_Imported/SpecialSkillsEffectsPack/AllEffects/EffectsSet_1(NotScriptBased)/Effects/Effect_41_Airstrike/Effect_41_Base/Effect_41_BulletExplosion.prefab";
        private const string AirRaidCloudSystemPrefabPath = "Assets/_Imported/AssetStore/_DLNK/Essential Terrain Pack/[PREFABS]/05 FX/Clouds/CloudSystem00.prefab";
        private const string AirRaidCloudTexturePath = "Assets/_Imported/AssetStore/_DLNK/Essential Terrain Pack/Source/Maps/[Texture Pack]/FX/Clouds/clouda.png";
        private const string AirRaidPreviewMaterialRoot = "Assets/_Game/Art/Materials/ActionFoundation/IntroGatePodAirRaid";
        private const string AirRaidJetPreviewMaterialPath = AirRaidPreviewMaterialRoot + "/AF_IntroAirRaidJetPreview.mat";
        private const string AirRaidBombPreviewMaterialPath = AirRaidPreviewMaterialRoot + "/AF_IntroAirRaidBombPreview.mat";
        private const string AirRaidSkyPreviewMaterialPath = AirRaidPreviewMaterialRoot + "/AF_IntroAirRaidSkyPreview.mat";
        private const string AirRaidCloudPreviewMaterialPath = AirRaidPreviewMaterialRoot + "/AF_IntroAirRaidCloudPreview.mat";
        private const string AirRaidEngineFlamePreviewMaterialPath = AirRaidPreviewMaterialRoot + "/AF_IntroAirRaidEngineFlamePreview.mat";
        private const string AirRaidPreviewCapturePath = "C:/Users/dharm/AppData/Local/Temp/intro-gatepod-airraid-prelude-preview.png";
        private const int AirRaidPreviewLayer = 2;
        private const int AirRaidPreviewCullingMask = 1 << AirRaidPreviewLayer;
        ...
        [MenuItem("DimensionBrawl/Apply Intro GatePod Air Raid Prelude Preview To Olympus Stage")]
        public static void ApplyIntroGatePodAirRaidPreludePreviewToOlympusStageMenu()
        {
            ApplyIntroGatePodAirRaidPreludePreviewToOlympusStage();
            Debug.Log("Applied intro GatePod air raid prelude preview to the Olympus corridor stage port.");
        }

        public static void ApplyIntroGatePodAirRaidPreludePreviewToOlympusStage()
        {
            StageDefinitionProfile profile = LoadRequired<StageDefinitionProfile>(DefinitionPath);
            Scene stageScene = EditorSceneManager.OpenScene(profile.MapScenePath, OpenSceneMode.Single);
            GameObject stageRoot = RequireRoot(stageScene, profile.MapRootName);
            ...
            if (FindDirectChild(introPayloadRoot, IntroGatePodGeneratedPayloadRootName) == null)
            {
                throw new InvalidOperationException(
                    "Intro GatePod visual payload is missing. Air raid preview must not regenerate it.");
            }

            RemoveChild(introPayloadRoot, IntroGatePodAirRaidPreludeRootName);
            GameObject previewRoot = CreateChild(
                introPayloadRoot,
                IntroGatePodAirRaidPreludeRootName,
                new Vector3(0f, 9.5f, 0f),
                Quaternion.identity,
                Vector3.one);

            BuildAirRaidPreviewAircraft(previewRoot.transform);
            BuildAirRaidPreviewImpacts(previewRoot.transform);
            BuildAirRaidPreviewBackdrop(previewRoot.transform);
            BuildAirRaidPreviewCamera(previewRoot.transform);
            BuildAirRaidPreviewNotes(previewRoot.transform);
            SetLayerRecursively(previewRoot, AirRaidPreviewLayer);

            ValidateAirRaidPreludePreview(previewRoot.transform);
        ...
        [MenuItem("DimensionBrawl/Render Intro GatePod Air Raid Prelude Preview")]
        public static void RenderIntroGatePodAirRaidPreludePreviewMenu()
        {
            string previewPath = RenderIntroGatePodAirRaidPreludePreview();
            Debug.Log($"Rendered intro GatePod air raid prelude preview: {previewPath}");
        }

        public static string RenderIntroGatePodAirRaidPreludePreview()
        {
            StageDefinitionProfile profile = LoadRequired<StageDefinitionProfile>(DefinitionPath);
            Scene stageScene = EditorSceneManager.OpenScene(profile.MapScenePath, OpenSceneMode.Single);
            GameObject stageRoot = RequireRoot(stageScene, profile.MapRootName);
            Transform introPayloadRoot = RequireIntroPortPayloadRoot(stageRoot.transform);
            Transform previewRoot = RequireChild(introPayloadRoot, IntroGatePodAirRaidPreludeRootName);
            Camera camera = RequireComponent<Camera>(RequireChild(previewRoot, IntroGatePodAirRaidCameraName).gameObject);

            SimulatePreviewParticles(previewRoot, 0.45f);
            RenderCameraToPng(camera, AirRaidPreviewCapturePath, 1280, 720);
            return AirRaidPreviewCapturePath;
        }

        private static void BuildAirRaidPreviewAircraft(Transform previewRoot)
        {
            Material jetMaterial = EnsureAirRaidJetMaterial();
            Material flameMaterial = EnsureAirRaidUnlitMaterial(
                AirRaidEngineFlamePreviewMaterialPath,
                new Color(1f, 0.46f, 0.12f, 1f),
                null,
                Vector2.one);
            Transform aircraftRoot = CreateChild(
                previewRoot,
                "Aircraft_ThreeJetFormation_12_883_15_200",
                Vector3.zero,
                Quaternion.identity,
                Vector3.one).transform;

            GameObject leadJet = InstantiatePreviewAsset(
                AirRaidJetModelPath,
                aircraftRoot,
                "Aircraft_Lead_Jet_04",
                new Vector3(10.8f, -0.08f, 0f),
                new Vector3(0f, 90f, 0f),
                new Vector3(0.48f, 0.48f, 0.48f));
            GameObject leftJet = InstantiatePreviewAsset(
                AirRaidJetModelPath,
                aircraftRoot,
                "Aircraft_LeftWing_Jet_04",
                new Vector3(8.55f, -0.35f, -2.65f),
                new Vector3(0f, 90f, 0f),
                new Vector3(0.4f, 0.4f, 0.4f));
            GameObject rightJet = InstantiatePreviewAsset(
                AirRaidJetModelPath,
                aircraftRoot,
                "Aircraft_RightWing_Jet_04",
                new Vector3(8.55f, -0.35f, 2.65f),
                new Vector3(0f, 90f, 0f),
                new Vector3(0.4f, 0.4f, 0.4f));

            ApplyPreviewMaterial(leadJet, jetMaterial);
            ApplyPreviewMaterial(leftJet, jetMaterial);
            ApplyPreviewMaterial(rightJet, jetMaterial);

            CreatePointLight(aircraftRoot, "EngineGlow_Lead", new Vector3(9.95f, -0.22f, 0f), new Color(1f, 0.43f, 0.16f), 1.4f, 3.2f);
            CreatePointLight(aircraftRoot, "EngineGlow_LeftWing", new Vector3(7.85f, -0.5f, -2.65f), new Color(1f, 0.43f, 0.16f), 1.2f, 2.4f);
            CreatePointLight(aircraftRoot, "EngineGlow_RightWing", new Vector3(7.85f, -0.5f, 2.65f), new Color(1f, 0.43f, 0.16f), 1.2f, 2.4f);
            CreatePointLight(aircraftRoot, "PreviewOnly_AircraftFill_CameraSide", new Vector3(-3.5f, 3.2f, 0f), new Color(0.78f, 0.84f, 0.88f), 22f, 3.8f);
            CreatePointLight(aircraftRoot, "PreviewOnly_AircraftFill_CloudSide", new Vector3(13f, 4.4f, 0f), new Color(0.58f, 0.68f, 0.82f), 28f, 2.8f);
            CreatePrimitiveChild(
                aircraftRoot,
                PrimitiveType.Sphere,
                "EngineFlame_Lead_12_883",
                new Vector3(9.72f, -0.17f, 0f),
                Vector3.zero,
                new Vector3(0.56f, 0.18f, 0.18f),
                flameMaterial);
            CreatePrimitiveChild(
                aircraftRoot,
                PrimitiveType.Sphere,
                "EngineFlame_LeftWing_12_883",
                new Vector3(7.7f, -0.47f, -2.65f),
                Vector3.zero,
                new Vector3(0.44f, 0.14f, 0.14f),
                flameMaterial);
            CreatePrimitiveChild(
                aircraftRoot,
                PrimitiveType.Sphere,
                "EngineFlame_RightWing_12_883",
                new Vector3(7.7f, -0.47f, 2.65f),
                Vector3.zero,
                new Vector3(0.44f, 0.14f, 0.14f),
                flameMaterial);
        ...
        private static void BuildAirRaidPreviewImpacts(Transform previewRoot)
        {
            Material aircraftMaterial = EnsureAirRaidJetMaterial();
            Material bombMaterial = EnsureAirRaidLitMaterial(
                AirRaidBombPreviewMaterialPath,
                new Color(0.12f, 0.12f, 0.1f, 1f),
                null,
                Vector2.one,
                metallic: 0.38f,
                smoothness: 0.42f);
            Transform impactRoot = CreateChild(
                previewRoot,
                "Impacts_BombingChain_15_200_16_400",
                Vector3.zero,
                Quaternion.identity,
                Vector3.one).transform;

            InstantiatePreviewAsset(
                AirRaidBombExplosionPrefabPath,
                impactRoot,
                "Impact_Primary_BombExplosion_15_200",
                new Vector3(20f, -5.8f, -4.25f),
                Vector3.zero,
                new Vector3(2.4f, 2.4f, 2.4f));
            InstantiatePreviewAsset(
                AirRaidBulletExplosionPrefabPath,
                impactRoot,
                "Impact_Secondary_BulletExplosion_15_650",
                new Vector3(23.5f, -5.2f, 3.35f),
                Vector3.zero,
                new Vector3(1.8f, 1.8f, 1.8f));
            GameObject bomber = InstantiatePreviewAsset(
                AirRaidBomberModelPath,
                impactRoot,
                "Bomber_02_BombRun_15_200",
                new Vector3(15f, 0.92f, -1.1f),
                new Vector3(0f, 90f, 0f),
                new Vector3(0.7f, 0.7f, 0.7f));
            GameObject bomb = InstantiatePreviewAsset(
                AirRaidBombModelPath,
                impactRoot,
                "Bomb_01_Drop_15_320",
                new Vector3(17.9f, -1.35f, -2.4f),
                new Vector3(12f, 0f, -8f),
                new Vector3(0.58f, 0.58f, 0.58f));
            InstantiatePreviewAsset(
                AirRaidWholeStrikePrefabPath,
                impactRoot,
                "Effect_41_Airstrike_WholeSkill_15_200",
                new Vector3(20f, -5.8f, -1.5f),
                Vector3.zero,
                new Vector3(0.28f, 0.28f, 0.28f));

            ApplyPreviewMaterial(bomber, aircraftMaterial);
            ApplyPreviewMaterial(bomb, bombMaterial);

            CreatePointLight(impactRoot, "ImpactFlash_Primary", new Vector3(20f, -4.8f, -4.25f), new Color(1f, 0.55f, 0.2f), 7f, 8f);
            CreatePointLight(impactRoot, "ImpactFlash_Secondary", new Vector3(23.5f, -4.4f, 3.35f), new Color(1f, 0.5f, 0.16f), 5f, 5.5f);
            impactRoot.gameObject.SetActive(false);
            EditorUtility.SetDirty(impactRoot);
        }

        private static void BuildAirRaidPreviewBackdrop(Transform previewRoot)
        {
            Material skyMaterial = EnsureAirRaidUnlitMaterial(
                AirRaidSkyPreviewMaterialPath,
                new Color(0.62f, 0.72f, 0.78f, 1f),
                null,
                Vector2.one);
            Material cloudMaterial = EnsureAirRaidTransparentUnlitMaterial(
                AirRaidCloudPreviewMaterialPath,
                new Color(0.88f, 0.93f, 0.92f, 0.48f),
                LoadRequired<Texture2D>(AirRaidCloudTexturePath),
                Vector2.one);

            Transform backdropRoot = CreateChild(
                previewRoot,
                "Backdrop_SkyCloudLayer_12_883_16_400",
                Vector3.zero,
                Quaternion.identity,
                Vector3.one).transform;
            ...
            CreatePrimitiveChild(
                backdropRoot,
                PrimitiveType.Quad,
                "SkyWall_PaleOvercast",
                new Vector3(42f, 4f, 0f),
                new Vector3(0f, 90f, 0f),
                new Vector3(110f, 52f, 1f),
                skyMaterial);
            GameObject nearClouds = InstantiatePreviewAsset(
                AirRaidCloudSystemPrefabPath,
                backdropRoot,
                "CloudSystem00_AssetCloudDeck_BelowFormation",
                new Vector3(20f, -8.15f, -0.8f),
                new Vector3(0f, -12f, 0f),
                new Vector3(4.9f, 0.36f, 3.1f));
            GameObject farClouds = InstantiatePreviewAsset(
                AirRaidCloudSystemPrefabPath,
                backdropRoot,
                "CloudSystem00_AssetCloudDeck_FarHaze",
                new Vector3(44f, -6.15f, 3.5f),
                new Vector3(0f, 21f, 0f),
                new Vector3(4.6f, 0.3f, 3.15f));
            ApplyPreviewMaterial(nearClouds, cloudMaterial);
            ApplyPreviewMaterial(farClouds, cloudMaterial);
        }

        private static void BuildAirRaidPreviewCamera(Transform previewRoot)
        {
            GameObject cameraObject = CreateChild(
                previewRoot,
                IntroGatePodAirRaidCameraName,
                new Vector3(-7.5f, 1.65f, 0f),
                Quaternion.identity,
                Vector3.one);

            cameraObject.transform.LookAt(previewRoot.TransformPoint(new Vector3(10.2f, 0.38f, 0f)), Vector3.up);
            Camera camera = cameraObject.AddComponent<Camera>();
            camera.enabled = false;
            camera.clearFlags = CameraClearFlags.SolidColor;
            camera.backgroundColor = new Color(0.66f, 0.71f, 0.72f);
            camera.cullingMask = AirRaidPreviewCullingMask;
        ...
        private static void BuildAirRaidPreviewNotes(Transform previewRoot)
        {
            Transform notesRoot = CreateChild(
                previewRoot,
                "TimingNotes_DoNotBindToMainTimeline",
                Vector3.zero,
                Quaternion.identity,
                Vector3.one).transform;

            CreateChild(notesRoot, "12_883_HardCut_FromProtectedHandLook", Vector3.zero, Quaternion.identity, Vector3.one);
            CreateChild(notesRoot, "12_883_15_200_ThreeJetRearFollowShot", Vector3.zero, Quaternion.identity, Vector3.one);
            CreateChild(notesRoot, "15_200_16_400_BombingImpactWithAdditiveRecoilOnly", Vector3.zero, Quaternion.identity, Vector3.one);
            CreateChild(notesRoot, "Protected_FirstPerson_Visuals_Timeline_NotEdited", Vector3.zero, Quaternion.identity, Vector3.one);
        }

        private static GameObject InstantiatePreviewAsset(
            string assetPath,
            Transform parent,
            string objectName,
            ...
            Vector3 localScale)
        {
            GameObject asset = LoadRequired<GameObject>(assetPath);
        ...
        private static Material EnsureAirRaidJetMaterial()
        {
            return EnsureAirRaidLitMaterial(
                AirRaidJetPreviewMaterialPath,
                new Color(0.86f, 0.9f, 0.92f, 1f),
                LoadRequired<Texture2D>(AirRaidJetTexturePath),
                new Vector2(1f, 1f),
                metallic: 0.18f,
                smoothness: 0.62f);
        }

        private static Material EnsureAirRaidLitMaterial(
            string materialPath,
            Color baseColor,
            Texture baseTexture,
        ...
        private static Material EnsureAirRaidUnlitMaterial(
            string materialPath,
            Color baseColor,
            Texture baseTexture,
            Vector2 textureScale)
        ...
        private static Material EnsureAirRaidTransparentUnlitMaterial(
            string materialPath,
            Color baseColor,
            Texture baseTexture,
            Vector2 textureScale)
        {
            Material material = EnsureAirRaidMaterial(
                materialPath,
        ...
        private static Material EnsureAirRaidMaterial(
            string materialPath,
            Shader shader,
            Color baseColor,
            Texture baseTexture,
            ...
            float smoothness)
        {
            if (shader == null)
            {
                throw new InvalidOperationException($"No compatible shader found for {materialPath}.");
            }

            EnsureFolder(AirRaidPreviewMaterialRoot);
        ...
        private static void CreatePointLight(
            ...
            float intensity)
        {
            GameObject lightObject = CreateChild(parent, objectName, localPosition, Quaternion.identity, Vector3.one);
        ...
        private static void ValidateAirRaidPreludePreview(Transform previewRoot)
        {
            RequireChild(previewRoot, IntroGatePodAirRaidCameraName);
            Transform aircraftRoot = RequireChild(previewRoot, "Aircraft_ThreeJetFormation_12_883_15_200");
            RequireChild(aircraftRoot, "Aircraft_Lead_Jet_04");
            RequireChild(aircraftRoot, "Aircraft_LeftWing_Jet_04");
            RequireChild(aircraftRoot, "Aircraft_RightWing_Jet_04");
            RequireChild(previewRoot, "Impacts_BombingChain_15_200_16_400");
            RequireChild(previewRoot, "Backdrop_SkyCloudLayer_12_883_16_400");

            int rendererCount = previewRoot.GetComponentsInChildren<Renderer>(includeInactive: true).Length;
            if (rendererCount < 3)
            {
                throw new InvalidOperationException("Air raid prelude preview has too few renderers for the three-aircraft shot.");
            }
        }

        private static void SimulatePreviewParticles(Transform previewRoot, float seconds)
        {
            ParticleSystem[] particleSystems = previewRoot.GetComponentsInChildren<ParticleSystem>(includeInactive: true);
            for (int i = 0; i < particleSystems.Length; i++)
            {
                particleSystems[i].Simulate(seconds, withChildren: false, restart: true, fixedTimeStep: true);
            }
        }

        private static void RenderCameraToPng(Camera camera, string outputPath, int width, int height)
        {
            bool previousActiveSelf = camera.gameObject.activeSelf;
            ...
            try
            {
                ...
                GL.Clear(clearDepth: true, clearColor: true, backgroundColor: camera.backgroundColor);
                camera.Render();
            ...
            finally
            {
                ...
                camera.gameObject.SetActive(previousActiveSelf);
