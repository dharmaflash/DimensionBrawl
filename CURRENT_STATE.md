...
- The first combat cue prefabs are safe game-owned mesh cue prefabs generated under `_Game/Art/VFX/CombatCues`; `CombatVfxCueVisual` drives their short scale/color/spin fade so ActionFoundation cues do not read as broken particle shards. They do not directly embed raw vendor-prefab dependencies from `_Imported`. If a vendor effect becomes the final visual choice, promote that specific source prefab plus required materials/textures/meshes in a reviewed slice before wiring it into the cue profile.
...
- `DimensionBrawl > Validate Action Foundation Combat VFX Cues` validates the combat VFX cue profile, promoted `_Game` prefab paths, mesh-based `CombatVfxCueVisual` prefabs with no first-pass particle shards, player/enemy cue-player bindings, eight pattern cue overrides, and five elite signal cue overrides.
...
Use `Assets/_Game/DesignDocs/COMBAT_V1_SPEC.md`, `Assets/_Game/DesignDocs/ACTION_FEEL_TARGETS.md`, `Assets/_Game/DesignDocs/ACTION_FOUNDATION_OWNERSHIP.md`, `Assets/_Game/DesignDocs/ACTION_FOUNDATION_TESTING.md`, and `Assets/_Game/DesignDocs/CINEMACHINE_INGAME_CUTSCENE_REFERENCE_RESEARCH.md` as implementation guardrails. The next safe step is to manually inspect the refreshed ActionFoundation combat cue readability in Play Mode, then either tune selected promoted cue shapes/materials or continue into the next gameplay slice; do not add summon behavior, waves, bosses, or progression before this combat-readability check is accepted.
